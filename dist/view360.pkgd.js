...
- @egjs/agent ^2.2.1, @egjs/axes ^2.7.1, @egjs/component ^2.1.2, es6-promise ^4.2.5, gl-matrix ^3.1.0, motion-sensors-polyfill ^0.3.1, webvr-polyfill ^0.9.41
*/
(function (global, factory) {
  ...
  */
  ...
  function execRegExp$1(pattern, text) {
    try {
      return new RegExp(pattern, "g").exec(text);
    } catch (e) {
      return null;
  ...
  function findVersion$1(versionTest, userAgent) {
    var result = execRegExp$1("(" + versionTest + ")((?:\\/|\\s|:)([0-9|\\.|_]+))?", userAgent);
    return result ? result[3] : "";
  }
  function convertVersion$1(text) {
    return text.replace(/_/g, ".");
  }
  function findPreset$1(presets, userAgent) {
    var userPreset = null;
    var version = "-1";
    some$1(presets, function (preset) {
      var result = execRegExp$1("(" + preset.test + ")((?:\\/|\\s|:)([0-9|\\.|_]+))?", userAgent);
      ...
      if (preset.versionAlias) {
        ...
        version = findVersion$1(preset.versionTest.toLowerCase(), userAgent) || version;
      }

      version = convertVersion$1(version);
      return true;
    });
    return {
      preset: userPreset,
  ...
  function findBrand$1(brands, preset) {
    return find$1(brands, function (_a) {
      var brand = _a.brand;
      return execRegExp$1("" + preset.test, brand.toLowerCase());
    });
  }

  var BROWSER_PRESETS$1 = [{
    test: "phantomjs",
    id: "phantomjs"
  }, {
    test: "whale",
  ...
  var WEBKIT_PRESETS$1 = [{
    test: "applewebkit",
    id: "webkit"
  }];
  var WEBVIEW_PRESETS$1 = [{
    test: "(?=(iphone|ipad))(?!(.*version))",
    id: "webview"
  }, {
    test: "(?=(android|iphone|ipad))(?=.*(naver|daum|; wv))",
  ...
  function parseUserAgentData$1(osData) {
    var userAgentData = navigator.userAgentData;
    var brands = (userAgentData.uaList || userAgentData.brands).slice();
    var isMobile = userAgentData.mobile || false;
    var firstBrand = brands[0];
    var browser = {
      ...
      majorVersion: -1,
      webkit: false,
      webview: some$1(WEBVIEW_PRESETS$1, function (preset) {
        return findBrand$1(brands, preset);
      }),
      chromium: some$1(CHROMIUM_PRESETS$1, function (preset) {
        return findBrand$1(brands, preset);
      })
    };
    var os = {
    ...
    browser.webkit = !browser.chromium && some$1(WEBKIT_PRESETS$1, function (preset) {
      return findBrand$1(brands, preset);
    });

    if (osData) {
      var platform_1 = osData.platform.toLowerCase();
      var result = find$1(OS_PRESETS$1, function (preset) {
        return new RegExp("" + preset.test, "g").exec(platform_1);
      });
      os.name = result ? result.id : platform_1;
      os.version = osData.platformVersion;
    ...
    some$1(BROWSER_PRESETS$1, function (preset) {
      var result = findBrand$1(brands, preset);
    ...
    os.version = convertVersion$1(os.version);
    browser.version = convertVersion$1(browser.version);
    os.majorVersion = parseInt(os.version, 10);
    browser.majorVersion = parseInt(browser.version, 10);
    return {
      browser: browser,
  ...
  function parseUserAgent$1(userAgent) {
    var nextAgent = getUserAgent$1(userAgent);
    var isMobile = !!/mobi/g.exec(nextAgent);
    var browser = {
      name: "unknown",
      version: "-1",
      majorVersion: -1,
      webview: !!findPreset$1(WEBVIEW_PRESETS$1, nextAgent).preset,
      chromium: !!findPreset$1(CHROMIUM_PRESETS$1, nextAgent).preset,
      webkit: false
    };
    var os = {
      name: "unknown",
    ...
    var _a = findPreset$1(BROWSER_PRESETS$1, nextAgent),
        browserPreset = _a.preset,
        browserVersion = _a.version;

    var _b = findPreset$1(OS_PRESETS$1, nextAgent),
        osPreset = _b.preset,
        osVersion = _b.version;

    browser.webkit = !browser.chromium && !!findPreset$1(WEBKIT_PRESETS$1, nextAgent).preset;
  ...
  function agent$2(userAgent) {
    if (typeof userAgent === "undefined" && hasUserAgentData$1()) {
      return parseUserAgentData$1();
    } else {
  ...
  */
  ...
  var IS_IOS_SAFARI = "ontouchstart" in win$2 && agent$2().browser.name === "safari";
  ...
  **/
  ...
  ComplementaryFilter.prototype.run_ = function() {
    ...
    if (util$1.isDebug()) {
      console.log('Delta: %d deg, G_est: (%s, %s, %s), G_meas: (%s, %s, %s)',
                  mathUtil.radToDeg * util$1.getQuaternionAngle(deltaQ),
  ...
  var agent$3 = agent();
  var isIE11 = agent$3.browser.name === "ie" && agent$3.browser.majorVersion === 11;
  var EVENTS = {
    ERROR: "error"
  };
  /**
  ...
  function () {
    ...
    function (_Renderer) {
      ...
      _proto.getMaxCubeMapTextureSize = function getMaxCubeMapTextureSize(gl, image) {
        ...
        if (agent$$1.browser.name === "ie" && agent$$1.browser.majorVersion === 11) {
        ...
        if (agent$$1.os.name === "ios") {
          var majorVersion = agent$$1.os.majorVersion; // ios 9 의 경우 텍스쳐 최대사이즈는 1024 이다.
