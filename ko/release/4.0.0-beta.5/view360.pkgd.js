...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.View360 = factory());
    ...
    ***************************************************************************** */
    ...
    const eulerToQuat = (out, yaw, pitch, roll) => {
      ...
      rotateY(out, out, yaw * DEG_TO_RAD);
      rotateX(out, out, pitchClamped * DEG_TO_RAD);
      rotateZ(out, out, roll * DEG_TO_RAD);
      return out;
    };
    /**
    ...
    const quatToEuler = quaternion => {
      ...
      if (test > 0.499995 * unit) {
        ...
        const view = fromValues$3(0, 0, 1);
        const up = fromValues$3(0, 1, 0);
        transformQuat(view, view, quaternion);
        transformQuat(up, up, quaternion);
    ...
    class CameraAnimation {
      ...
      update(deltaTime) {
        ...
        slerp(rotation, from.rotation, to.rotation, progress);
    ...
    class Camera extends Component {
      ...
      lookAt({
        ...
        const prevQuaternion = clone(this.quaternion);
        ...
        if (!equals(this.quaternion, prevQuaternion) || zoomDiff >= EPSILON * 10 // ignore small changes
        ) {
          this.updateMatrix();
        }
      }
      ...
      rotate(rotation, zoom = this.zoom) {
        const normalized = normalize(create$1(), rotation);
        const isSameRotation = equals(this.quaternion, normalized);
        copy(this.quaternion, normalized);
        const prevZoom = this.zoom;
        const {
          yaw,
          pitch
      ...
      animateTo({
        ...
        return __awaiter(this, void 0, void 0, function* () {
          ...
          const from = {
            rotation: clone(this.quaternion),
            zoom: this.zoom
          };
          const to = {
            rotation: eulerToQuat(create$1(), yaw, pitch, this.rollOffset),
            zoom
          };
      ...
      updateMatrix() {
        ...
        const upDir = create$3();
        const viewDir = fromValues$3(0, 0, -1);
        transformQuat(viewDir, viewDir, rotation);
        transformQuat(upDir, up, rotation);
        const hFov = this._getZoomedHorizontalFov(); // In radians
        const vFov = toVerticalFov(hFov, aspect);
        lookAt(viewMatrix, position, viewDir, upDir);
        perspective(projMatrix, vFov, aspect, 0.1, 100);
        this._changed = true;
      }
      /**
       * @hidden
    ...
    */
    ...
    class GyroInput extends Component {
      ...
      collectDelta() {
        ...
        const prevRotation = clone(this.quaternion);
        this._updateRotation();
        this._orientationUpdated = false;
      ...
      _updateRotation() {
        ...
        identity(rotation);
        rotateY(rotation, rotation, (alpha - this._yawOffset) * DEG_TO_RAD);
        rotateX(rotation, rotation, beta * DEG_TO_RAD);
        rotateZ(rotation, rotation, -gamma * DEG_TO_RAD);
        const screen = create$1();
        const screenAngle = -this._screenOrientation * 0.5 * DEG_TO_RAD;
        const world = fromValues$1(-Math.sqrt(0.5), 0, 0, Math.sqrt(0.5));
        set(screen, 0, Math.sin(screenAngle), 0, Math.cos(screenAngle));
        multiply(rotation, rotation, screen);
        multiply(rotation, rotation, world);
      ...
      _getRotationDelta(prevQ, curQ, rotateKind) {
        const targetAxis = fromValues$3(ROTATE_CONSTANT[rotateKind].targetAxis[0], ROTATE_CONSTANT[rotateKind].targetAxis[1], ROTATE_CONSTANT[rotateKind].targetAxis[2]);
        const meshPoint = ROTATE_CONSTANT[rotateKind].meshPoint;
        const prevQuaternion = clone(prevQ);
        const curQuaternion = clone(curQ);
        normalize(prevQuaternion, prevQuaternion);
        normalize(curQuaternion, curQuaternion);
        let prevPoint = fromValues$3(0, 0, 1);
        let curPoint = fromValues$3(0, 0, 1);
        ...
        const rotateDistance = dot(targetAxis, cross(create$3(), prevPoint, curPoint));
        ...
        const meshPoint2 = fromValues$3(meshPoint[0], meshPoint[1], meshPoint[2]);
        let meshPoint3;
        if (rotateKind !== ROTATE_CONSTANT.YAW_DELTA_BY_YAW) {
          meshPoint3 = fromValues$3(0, rotateDirection, 0);
        } else {
          meshPoint3 = fromValues$3(rotateDirection, 0, 0);
        }
        ...
        normalize$2(vecN, vecN);
        const coefficientA = vecN[0];
        const coefficientB = vecN[1];
        const coefficientC = vecN[2];
        // a point on the plane
        curPoint = fromValues$3(meshPoint[0], meshPoint[1], meshPoint[2]);
        transformQuat(curPoint, curPoint, curQuaternion);
        // a point should project on the plane
        prevPoint = fromValues$3(meshPoint[0], meshPoint[1], meshPoint[2]);
        transformQuat(prevPoint, prevPoint, prevQuaternion);
        ...
        const projectedPrevPoint = create$3();
        subtract(projectedPrevPoint, prevPoint, scale(create$3(), vecN, distance));
        let trigonometricRatio = (projectedPrevPoint[0] * curPoint[0] + projectedPrevPoint[1] * curPoint[1] + projectedPrevPoint[2] * curPoint[2]) / (length(projectedPrevPoint) * length(curPoint));
        // defensive block
        if (trigonometricRatio > 1) {
          trigonometricRatio = 1;
        }
        const theta = Math.acos(trigonometricRatio);
        const crossVec = cross(create$3(), curPoint, projectedPrevPoint);
      ...
      _extractPitchFromQuat(quaternion) {
        const baseV = fromValues$3(0, 0, 1);
    ...
    */
    ...
    class TextureLoader {
      constructor() {
        this._loadChecker = new ImReady$1();
      }
      load(src, video) {
        return __awaiter(this, void 0, void 0, function* () {
          if (video) {
    ...
    class HotspotRenderer {
      ...
      render(camera) {
        ...
        hotspots.forEach(hotspot => {
          ...
          const relPos = create$3();
          copy$2(relPos, position);
          transformMat4(relPos, relPos, camera.viewMatrix);
          transformMat4(relPos, relPos, camera.projectionMatrix);
          if (relPos[2] > 1 || relPos[2] < 0) {
            hotspot.element.classList.remove(DEFAULT_CLASS.HOTSPOT_VISIBLE);
            return;
          }
          const screenPos = fromValues(relPos[0] * halfWidth + halfWidth, -relPos[1] * halfHeight + halfHeight);
      ...
      _parseHotspot(element) {
        ...
        if (yawStr || pitchStr) {
          ...
          return new Hotspot(element, fromValues$3(pos[0], pos[1], pos[2]));
        } else {
          // Place hotspot at yaw: 0, pitch: 0
          const defaultPos = fromValues$3(0, 0, -1);
    ...
    class WebGLContext {
      ...
      updateCommonUniforms(entity, camera, shaderProgram) {
        ...
        const mvMatrix = create$4();
        multiply$1(mvMatrix, camera.viewMatrix, matrix);
    ...
    class WebGLRenderer {
      ...
      renderVR(projection, vr, frame) {
        ...
        eyeParams.forEach((eye, eyeIndex) => {
          ...
          // As projection doesn't require world matrix, as it doesn't have any parent or child
          const mvMatrix = multiply$1(create$4(), eye.vMatrix, mesh.matrix);
    ...
    class Object3D {
       ...
       */
      constructor() {
        this.matrix = create$4();
        this.rotation = create$1();
        this.position = fromValues$3(0, 0, 0);
       ...
       */
      updateMatrix() {
        fromRotationTranslationScale(this.matrix, this.rotation, this.position, this.scale);
      }
    }
    ...
    */
    ...
    class CylindricalProjection extends Projection {
      ...
      applyTexture(ctx, texture) {
        ...
        identity(mesh.rotation);
        rotateY(mesh.rotation, mesh.rotation, -Math.PI / 2);
        mesh.updateMatrix();
        this._mesh = mesh;
      }
      updateCamera(camera) {
